...
Private Sub SkipCurrentRecordAndAdvance(ByVal frm As Form)
    On Error GoTo EH

    If IsNull(frm!ReviewID) Then Exit Sub
    ...
    Exit Sub
EH:
    MsgBox "Überspringen konnte nicht abgeschlossen werden: " & Err.Description, vbExclamation
End Sub
...
Private Sub UpdateExistingSchuelerRecord(ByVal schuelerId As Long, ByVal frm As Form)
    ...
    UpdateFieldTimeFlexible rs, "SMovon", GetFormFieldValue(frm, "SMovon")
    UpdateFieldTimeFlexible rs, "SDivon", GetFormFieldValue(frm, "SDivon")
    UpdateFieldTimeFlexible rs, "SMivon", GetFormFieldValue(frm, "SMivon")
    UpdateFieldTimeFlexible rs, "SDovon", GetFormFieldValue(frm, "SDovon")
    UpdateFieldTimeFlexible rs, "SFrvon", GetFormFieldValue(frm, "SFrvon")
...
Private Sub InsertCurrentReviewRecord(ByVal frm As Form)
    ...
    sql = sql & SqlTimeForTarget("SMovon", GetFormFieldValue(frm, "SMovon")) & ","
    sql = sql & SqlTimeForTarget("SDivon", GetFormFieldValue(frm, "SDivon")) & ","
    sql = sql & SqlTimeForTarget("SMivon", GetFormFieldValue(frm, "SMivon")) & ","
    sql = sql & SqlTimeForTarget("SDovon", GetFormFieldValue(frm, "SDovon")) & ","
    sql = sql & SqlTimeForTarget("SFrvon", GetFormFieldValue(frm, "SFrvon")) & ")"
        ...
        Select Case MsgBox(promptTxt, vbYesNoCancel + vbExclamation, "Dubletten-Prüfung")
            ...
            Case vbCancel
                If ConfirmSkipOrAbort("Dubletten-Prüfung") Then
                    SkipCurrentRecordAndAdvance frm
                    Exit Sub
                Else
                    Exit Sub
                End If
...
Private Function SqlTimeForTarget(ByVal targetField As String, ByVal v As Variant) As String
    If IsTargetFieldText(C_TARGET_TABLE, targetField) Then
        SqlTimeForTarget = SqlText(TimeToTextHHNN(v))
    Else
        SqlTimeForTarget = SqlDateTime(TimeToDateValue(v))
    End If
End Function

Private Function TimeToDateValue(ByVal v As Variant) As Variant
    Dim s As String

    If IsNull(v) Then
        TimeToDateValue = Null
        Exit Function
    End If

    If IsDate(v) Then
        TimeToDateValue = CDate(v)
        Exit Function
    End If

    If IsNumeric(v) Then
        TimeToDateValue = DateSerial(1899, 12, 30) + CDbl(v)
        Exit Function
    End If
    ...
    s = Replace(s, ".", ":")
    If IsDate(s) Then
        TimeToDateValue = CDate(s)
    Else
        TimeToDateValue = Null
    End If
End Function

Private Function TimeToTextHHNN(ByVal v As Variant) As Variant
    Dim t As Variant
    t = TimeToDateValue(v)
    ...
    If IsNull(t) Then
        TimeToTextHHNN = Null
    Else
        TimeToTextHHNN = Format$(CDate(t), "hh:nn")
    End If
End Function
...
Private Sub UpdateFieldTimeFlexible(ByVal rs As DAO.Recordset, ByVal fieldName As String, ByVal value As Variant)
    Dim t As Variant

    If Not HasField(rs, fieldName) Then Exit Sub

    t = TimeToDateValue(value)
    If IsNull(t) Then Exit Sub

    Select Case rs.Fields(fieldName).Type
        Case dbText, dbMemo
            rs.Fields(fieldName).Value = Format$(CDate(t), "hh:nn")
        Case Else
            rs.Fields(fieldName).Value = CDate(t)
    End Select
End Sub

Private Function IsTargetFieldText(ByVal tableName As String, ByVal fieldName As String) As Boolean
    On Error GoTo EH

    Dim f As DAO.Field
    Set f = CurrentDb.TableDefs(tableName).Fields(fieldName)

    Select Case f.Type
        Case dbText, dbMemo
            IsTargetFieldText = True
    End Select
    Exit Function
EH:
    IsTargetFieldText = False
End Function
...
Private Sub SetFormFieldValue(ByVal frm As Form, ByVal fieldName As String, ByVal v As Variant)
    ...
    If HasField(frm.Recordset, fieldName) Then
        frm.Recordset.Edit
        frm.Recordset.Fields(fieldName).Value = v
    ...
    Exit Sub
EH:
    ' Feldsetzen darf Hauptlogik nicht abbrechen
End Sub
