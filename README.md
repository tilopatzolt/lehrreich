...
- Bei `hard`/`soft`: Auswahl über `Ja=Merge`, `Nein=Neu anlegen`, `Abbrechen=Überspringen` (mit zusätzlicher Sicherheitsabfrage für Überspringen).
  Bei bestätigtem `Überspringen` wird der Datensatz sofort als verarbeitet markiert und direkt zum nächsten Datensatz gewechselt.
...
Hinweis zu Wochenzeiten (`SMovon`…`SFrvon`): Wenn die Zielspalten in `Schueler` als Text definiert sind, werden Zeiten im Format `hh:nn` gespeichert (z. B. `14:15`) statt als `00:00:00`.
